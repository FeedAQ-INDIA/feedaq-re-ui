...
export default function PGSearchPage({reference, initialPage, lim}) {

    const [listingData, setListingData] = useState([]);
    const [selectedAddress, setSelectedAddress] = useState("");
    const [coordinates, setCoordinates] = useState({});
    const [mapReference, setMapReference] = useState(reference || 5000039876089);
    const [limit, setLimit] = useState(lim || 20);
    ...
    const findNearbyLocations = async (lat, lng) => {
        apiClient(`http://localhost:8080/searchPG?lat=${lat}&lng=${lng}&radius=5&limit=${limit}&page=${page}`)
