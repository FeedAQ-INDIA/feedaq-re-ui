...
export default function HomeProjectView({reference, initialPage, lim}) {
    ...
    const {
     ...
     const findNearbyLocations =  () => {
         apiClient(`http://localhost:8080/searchProject?lat=${latitude}&lng=${longitude}&radius=5&limit=${4}&page=${1}`)
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 my-6 gap-4">
            {listings?.map(a => (<ProjectListingCard listing={a}/>))}
            ...
            <Link href={`/project/search?reference=${mapReference}`} target="_blank"><div className="col-span-1 w-full h-[305px] rounded-md flex flex-col items-center justify-center bg-rose-500 ">
