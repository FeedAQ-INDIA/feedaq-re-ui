...
export default function HomePropertyView() {
    ...
    const {
     ...
     const findNearbyLocations =  () => {
         apiClient(`http://localhost:8080/search?transactionType=buy&lat=${latitude}&lng=${longitude}&radius=10&limit=${4}&page=${1}`)
            .then(res => res.json())               // ✅ Parse the response and return the Promise
            .then((json) => {
                console.log(json)
                setListings(json?.properties)
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 my-6 gap-4">
            {listings?.map(a => (<PropertyListingCard listing={a}/>))}
            ...
            <Link href={`/search?reference=${mapReference}`} target="_blank"><div className="col-span-1 w-full h-[325px] rounded-md flex flex-col items-center justify-center bg-rose-500 ">
